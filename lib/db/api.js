...
export const getUserGroups = async function(email) {
    return trace('getUserGroups', {email}, async() => {
        ...
        return await client.collection('groups').find(pipeline).toArray()
    })
}
...
export const returnDevicesToOriginGroup = function(serials) {
    return trace('returnDevicesToOriginGroup', {serials}, () => {
        return Promise.each(serials, function(serial) {
            return db.connect().then(client => {
                return client.collection('devices').findOne({serial: serial}).then(device => {
                    return client.collection('groups').findOne({id: device.group.origin}).then(async group => {
                        let originGroup = group || await getRootGroup()
                        return client.collection('devices').updateOne(
                            {serial: serial}
                            , {
                                $set: {
...
export const deleteGroup = async function(id) {
    return trace('deleteGroup', {id}, async() => {
        const client = await db.connect()
        return await client.collection('groups').deleteOne({id: id, name: {$ne: 'Common'}})
    })
}
...
export const setDeviceCapabilities = function(message) {
    return trace('setDeviceCapabilities', {message}, () => {
        return db.connect().then(client => {
            let setObj = {
                capabilities: {
...
const findDevice = function(condition, fields) {
    return db.connect().then(async client => {
        ...
        if (Object.keys(condition).includes('serial')) {
            return await findOneWithFields(collection, condition)
        }
...
export const loadBookableDevicesWithFiltersLock = function(groups, abi, model, type, sdk, version, devicesFunc, limit = null) {
    return trace('loadBookableDevicesWithFiltersLock', {groups, abi, model, type, sdk, version, devicesFunc, limit}, () => {
        ...
        return db.connect().then(client => {
            ...
            return client.collection('devices').aggregate(pipeline).toArray()
                .then(devices => {
...
export const loadAccessTokenByTitle = function(email, title) {
    return trace('loadAccessToken', {email, title}, () => {
        return db.connect().then(client => {
            return client.collection('accessTokens').findOne({email, title})
        })
    })
}
...
export const sendEvent = function(eventType, eventDetails, linkedEntities, timestamp) {
    return trace('sendEvent', {eventType, eventDetails, linkedEntities, timestamp}, () => {
        return db.connect().then(client => {
            return client.collection('statistics').insertOne({
                eventType: eventType
...
export default {
    ...
    , loadAccessTokens
    , loadAccessToken
    , loadAccessTokenByTitle
    , grantAdmin
    , revokeAdmin
    , makeOriginGroupBookable
    , acceptPolicy
    , writeStats: sendEvent
    , getDevicesCount
