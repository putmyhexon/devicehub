...
export const addGroupDevices = async(group, serials) => {
    ...
    for (const device of devices) {
        if (isOriginGroup(device.group?.class)) {
            continue
        }

        const stop = new Date(device.group?.lifeTime?.stop || '')
        const start = new Date(device.group?.lifeTime?.start || '')
    ...
    if (group.class === apiutil.ONCE) {
        await AllModel.updateDevicesCurrentGroup(serials, group)
    }
...
export const deleteUserGroup = async(id) => {
    ...
    await AllModel.returnDevicesToOriginGroup(group?.devices)

    GroupChangeHandler.sendGroupChange(group, group?.users, false, false, false, [], false, [], 'deleted')

    // TODO: need?
    // GroupChangeHandler.treatGroupDevicesChange(group, null, group?.devices, false)
    // GroupChangeHandler.treatGroupUsersChange(group, group?.users, group?.isActive)
