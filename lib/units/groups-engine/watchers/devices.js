  ...
  db.connect().then(client => {
    ...
    changeStream.on('change', next => {
      ...
      try {
        ...
        return dbapi.getGroup(id).then(group => {
          ...
          if (group) {
            ...
            const isChangeLifeTime =
              newDoc.group.lifeTime.start.getTime() !==
            ...
            if (isChangeOriginGroup) {
              dbapi.getGroup(oldDoc.group.origin).then(function(originGroup) {
                if (originGroup) {
                  dbapi.removeOriginGroupDevice(originGroup, newDoc.serial)
                }
              })
              dbapi.getGroup(newDoc.group.origin).then(function(originGroup) {
                if (originGroup) {
                  dbapi.addOriginGroupDevice(originGroup, newDoc.serial)
                }
              })
              if (!isChangeCurrentGroup) {
                sendDeviceGroupChange(
