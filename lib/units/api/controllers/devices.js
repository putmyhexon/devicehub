...
function validateDeviceAccess(req, res) {
    const {deviceSerial, token} = req.body

    if (!deviceSerial || !token) {
        return res.status(400).json({
            success: false
            , message: 'deviceSerial and token are required'
        })
    }

    validateUserDeviceAccess(token, deviceSerial, req.options.secret)
        .then(result => {
        ...
        .catch(err => {
            ...
            log.error('Device access validation error:', error)
            res.status(500).json({
                success: false
...
/**
 * Check if user can control device via WebSocket (more permissive than isOwnedByUser)
 * Allows access if user is the device owner AND device is either:
 * - Currently being used by this user, OR
 * - Not being used by anyone (available for use)
 */
function canUserControlDevice(device, user) {
    return device.present &&
        device.ready &&
        device.owner &&
        device.owner.email === user.email &&
        (!device.using || device.using === user.email)
}

async function validateUserDeviceAccess(token, deviceSerial, secret, retryCount = 0) {
    try {
        const data = jwtutil.decode(token, secret)
        if (!data?.email) {
        ...
        const device = await dbapi.loadDevice(user.groups.subscribed, deviceSerial)
        if (!device) {
            throw new Error('Device not found')
        }

        log.info('Device ownership check - Device: %j', {
        ...
        log.info('User info: %j', {
            email: user.email
            , privilege: user.privilege
        })

        // Log each condition check for debugging
        log.info('Validation checks: present=%s, ready=%s, hasOwner=%s, ownerMatch=%s, notUsedOrUsedByUser=%s',
            device.present,
            device.ready,
            !!device.owner,
            device.owner?.email === user.email,
        ...
        // Handle race condition: if device has no owner yet but device is available,
        // wait briefly and retry in case ownership is being assigned by this user's UI action
        if (!device.owner && device.present && device.ready && !device.using && retryCount < 2) {
            log.info('Device has no owner yet, retrying in 100ms (attempt %d/2)', retryCount + 1)
            await new Promise(resolve => setTimeout(resolve, 100))
            return validateUserDeviceAccess(token, deviceSerial, secret, retryCount + 1)
        }

        // For WebSocket connections, use more permissive logic than regular API access
        if (!canUserControlDevice(device, user)) {
            log.warn('Device access denied. Device owner: %s, User: %s, User privilege: %s, Device using: %s',
                device.owner?.email, user.email, user.privilege, device.using)
            throw new Error('User does not have access to this device')
        }

        return {user, device, authorized: true}
    }
    catch (error) {
