      ...
      .then(autotestsGroup => {
        if (isInternal) {
          dbapi.loadDevicesBySerials(autotestsGroup.devices)
            .then(
              devices => apiutil.respond(res, 200, `Added (group ${target})`,
                {group: {id: autotestsGroup.id, devices: devices}})
            )
        }
        else {
          apiutil.respond(res, 200, `Added (group ${target})`
            , {group: apiutil.publishGroup(autotestsGroup)})
        }
      })
      .catch(function(err) {
        log.error(err)
  ...
  lockutil.lockGroup(req, res, lock).then(function(lockingSuccessed) {
    if (lockingSuccessed) {
      ...
      return (function() {
        ...
        if (amount) {
          return dbapi.loadBookableDevicesWithFiltersLock(
    ...
    if (lockingSuccessed) {
      ...
      return dbapi.removeGroupDevices(group, serialsToRemove).then(function(group) {
        if (group.class === apiutil.ONCE) {
          Promise.each(serialsToRemove, function(serial) {
            dbapi.updateDeviceCurrentGroup(serial, group)
          })
            .then(() => {
              if (isInternal) {
                return true
    ...
    function unsetDevices(serials) {
      Promise.map(serials, function(serial) {
        let unsetReq = {
          ...
          , serial: serial
          , user: req.user
          , swagger: {params: {email: {value: req.user.email}}}
          , options: req.options
        }
        ...
        if (group) {
          if (isInternal) {
            dbapi.loadDevicesBySerials(group.devices)
              .then(
                devices => apiutil.respond(res, 200, 'Updated (group)',
                  {group: {id: group.id, devices: devices}})
              )
          }
          else {
      ...
      if (name === group.name &&
        ...
        if (isInternal) {
          return dbapi.loadDevicesBySerials(group.devices)
            .then(
              devices => apiutil.respond(res, 200, 'Unchanged (group)', {group: {id: group.id, devices: devices}})
            )
        }
        else {
