...
const log = logger.createLogger('users-controller:')
...
function grantAdmin(email, res) {
  dbapi.grantAdmin(email)
  return apiutil.respond(res, 200)
}

function revokeAdmin(email, res) {
  dbapi.revokeAdmin(email)
  return apiutil.respond(res, 200)
}
...
function grantAdminAccess(req, res) {
  grantAdmin(req.swagger.params.email.value, res)
}

function revokeAdminAccess(req, res) {
  revokeAdmin(req.swagger.params.email.value, res)
}
