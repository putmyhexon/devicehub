...
async function removeUser(email, req, res) {
    ...
    async function removeGroupUser(owner, id) {
        const userGroup = await dbapi.getUserGroup(owner, id)
        if (!userGroup) {
            return 'not found'
        }
        return owner === email ?
...
function grantAdmin(req, res) {
    ...
    dbapi.grantAdmin(req.params.email).then((user) => {
        if (user) {
            // @ts-ignore
...
function revokeAdmin(req, res) {
    ...
    dbapi.revokeAdmin(req.params.email).then((user) => {
        if (user) {
            // @ts-ignore
...
function lockStfAdminUser(res) {
    return dbapi.lockUser(apiutil.STF_ADMIN_EMAIL).then(function(stats) {
        if (stats.modifiedCount === 0) {
            return apiutil.lightComputeStats(res, stats)
        }
...
function getUsersAlertMessage(req, res) {
    const fields = req.query.fields
    return dbapi.loadUser(apiutil.STF_ADMIN_EMAIL).then(async function(user) {
