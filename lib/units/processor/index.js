    ...
    devDealer.on('message', wirerouter()
        ...
        dbapi
            ...
            appDealer.send([
                ...
                , wireutil.envelope(new wire.InstalledApplications(message.serial, wireutil.toInstalledApps(installedApps)))
            ])
        })
            .catch(err => {
            log.error(err)
        ...
        dbapi
            ...
            if (condition === null || condition.present === false) {
                devDealer.send([message.channel
                    , wireutil.envelope(new wire.ConnectDeviceViaUSB(message.id))
                ])
            }
        })
            .catch(err => {
