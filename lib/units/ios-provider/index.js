...
export default async function(options) {
    ...
    const startWorker = async(udid) => {
        ...
        const [wdaStopForwarding, screenStopForwarding] = await Promise.all([openPort(8100, wdaPort, udid), openPort(9100, screenPort, udid), options.usbmuxPath])

        const stopWda = await wda.startWda(udid)
