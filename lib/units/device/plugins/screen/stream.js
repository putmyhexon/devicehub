...
import {validateDeviceAccess} from '../../../../util/device-auth.js'
...
export default syrup.serial()
    ...
    .dependency(display)
    .dependency(options)
    .define(function(options, adb, router, minicap, scrcpy, display, screenOptions) {
        ...
        return createServer()
            .then(function(wss) {
                ...
                wss.on('connection', async function(ws, req) {
                    ...
                    // Extract token from WebSocket subprotocols (most secure) or headers
                    let token = null

                    log.info('WebSocket connection URL: %s', req.url)
                    log.info('WebSocket headers: %j', req.headers)

                    // Try to get token from WebSocket subprotocols first (most secure for browsers)
                    if (ws.protocol && ws.protocol.startsWith('access_token.')) {
                        token = ws.protocol.substring('access_token.'.length)
                        log.info('Token extracted from WebSocket subprotocol')
                    }

                    // Fallback: Try Authorization header (for programmatic clients)
                    ...
                    const apiUrl = 'http://localhost:7106' // TODO: Get from options
                    log.info('Making API request to: %s/api/v1/devices/validate-access', apiUrl)
                    await validateDeviceAccess(token, options.serial, apiUrl)
                        .then(result => {
                            if (result.authorized) {
                                authenticated = true
                                user = result.user
                                log.info('WebSocket authenticated for user %s on device %s', user.email, options.serial)
                                ...
                                pingTimer = setInterval(wsPingNotifier, 10 * 60000)
                            }
                            else {
                                log.warn('WebSocket authentication failed for device %s: %s', options.serial, result.error)
                                ws.send(JSON.stringify({
                                    type: 'auth_error'
                    ...
                    ws.on('message', function(data) {
                        if (!authenticated) {
                            log.warn('Received message from unauthenticated WebSocket connection')
                    ...
                    ws.on('close', function() {
                        ...
                        broadcastSet.remove(id)
                        if (authenticated) {
                            log.info('Authenticated WebSocket closed for user %s on device %s', user.email, options.serial)
                        }
                    })
                    if (options.needScrcpy) {
                    ...
                    ws.on('error', function(e) {
                        ...
                        broadcastSet.remove(id)
                        if (authenticated) {
                            log.error('WebSocket error for user %s on device %s: %s', user.email, options.serial, e.message)
                        }
                    })
                })
