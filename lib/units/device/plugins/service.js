...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push, sdk, apk) {
    ...
    function callService(intent) {
      ...
      return adb.getDevice(options.serial).shell(util.format(
        'am %s --user 0 %s'
    ...
    function openService() {
      ...
      return callService(util.format(
        ...
        .then(function() {
          devutil.executeShellCommand(adb,
            options.serial,
            'settings put system screen_brightness 0')
          devutil.executeShellCommand(adb, options.serial,
    ...
    plugin.getMobileServices = function() {
      return runServiceCommand(
        apk.wire.MessageType.GET_SERVICES
        , new apk.wire.GetServicesAvailabilityRequest()
      )
        .timeout(15000)
        .then(function(data) {
    ...
    return openAgent()
      .then(openService)
      .then(function() {
        // devutil.executeShellCommand(adb, options.serial, 'pm grant jp.co.cyberagent.stf android.permission.DUMP')
        router
