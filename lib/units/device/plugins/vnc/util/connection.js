...
var debug = debug$0('vnc:connection')
function VncConnection(conn, options) {
    ...
    this._serverSupportedSecurityByType =
        this.options.security.reduce(function(map, method) {
...
VncConnection.prototype._auth = function(type, data) {
    ...
    this._blockingOps.push(security.auth(data).bind(this)
        .then(function() {
        ...
        .catch(function() {
        this._writeSecurityResult(VncConnection.SECURITYRESULT_FAIL, 'Authentication failure')
        this.end()
    }))
}
                ...
                case VncConnection.STATE_NEED_CLIENT_SECURITY:
                    if ((chunk = this._consume(1))) {
                        if ((this._clientSecurity = this._parseSecurity(chunk)) === null) {
                            this._writeSecurityResult(VncConnection.SECURITYRESULT_FAIL, 'Unimplemented security type')
                            this.end()
                            return
                        }
                        debug('client security', this._clientSecurity)
                        if (!(this._clientSecurity in this._serverSupportedSecurityByType)) {
                            this._writeSecurityResult(VncConnection.SECURITYRESULT_FAIL, 'Unsupported security type')
                            this.end()
                            return
                        }
                    ...
                    if ((chunk = this._consume(1))) {
                        switch (chunk[0]) {
                            ...
                            case VncConnection.CLIENT_MESSAGE_CLIENTCUTTEXT:
                                ...
                                break
                            default:
                                this._error(new Error(util.format('Unsupported message type %d', chunk[0])))
                                return
                        }
                    }
                    break
