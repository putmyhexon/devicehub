    ...
    .define(function(options, adb, router, push) {
        ...
        router.on(wire.InstallMessage, function(channel, message) {
            ...
            async function pushApp(channel) {
                ...
                try {
                    log.info('Removing existing file if present')
                    // const rmOutput =
                    await adb.getDevice(options.serial).shell(['rm', '-f', target])
                    // await readAll(rmOutput)
                    log.info('Cleanup completed')
                }
                catch (error) {
                    log.warn('Cleanup failed (may be expected):', error instanceof Error ? error.message : String(error))
                }
                ...
                transfer.on('progress', (stats) => {
                    log.info(`Push progress: ${stats.bytesTransferred} bytes transferred`)
                })
                transfer.on('error', (error) => {
                    log.error('Push transfer error:', error)
                })
                ...
                try {
                    ...
                    await destination.cleanup()
                    log.error('Failed to verify pushed file:', error instanceof Error ? error.message : String(error))
                    throw error
                }
            }
            // Progress 0%
