...
const SeqQueue = require('../../../../wire/seqqueue')
    ...
    TouchConsumer.prototype.touchDown = function(point) {
      this._queueWrite(function() {
        return this._write(util.format(
          'd %s %s %s %s\n'
        , point.contact
        , Math.ceil(this.touchConfig.origin.x(point) * this.banner.maxX)
        , Math.ceil(this.touchConfig.origin.y(point) * this.banner.maxY)
        , Math.ceil((point.pressure || 0.5) * this.banner.maxPressure)
        ))
      })
    }
