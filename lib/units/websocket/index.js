...
const request = Promise.promisifyAll(require('postman-request'))
...
const ip = require('./middleware/remote-ip')
...
module.exports = function(options) {
  ...
  const io = require('socket.io')(server, io_options)
    ...
    new Promise(function(resolve) {
      ...
      socket.on('disconnect', resolve)
        ...
        .on('quality.change', function(channel, data) {
          push.send([
            channel
            , wireutil.envelope(new wire.ChangeQualityMessage(
              data.quality
            ))
          ])
        })
